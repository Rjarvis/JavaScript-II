...
function myFunction(value){
    txt = value.first_name + " " + value.last_name;
    fullName.push(txt);
}
...
let allCaps = [];
runners.map(myMapFunction);
function myMapFunction(value){
    txt = value.first_name.toUpperCase();
    allCaps.push(txt)
}
console.log(allCaps); 
...
let largeShirts = [];
runners.filter(myFilterFunction);
function myFilterFunction(value){
    return (value.shirt_size === "L") ? largeShirts.push(value):null;
}
...
ticketPriceTotal = runners.reduce(myReduceFunction, 0);
function myReduceFunction(total,value){
    // let pledge = value.pledge;
    // total + pledge;
    // return console.log(total);
    
    return total + parseInt(value.donation);   
}
